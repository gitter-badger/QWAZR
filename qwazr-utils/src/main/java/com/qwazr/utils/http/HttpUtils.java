...
public class HttpUtils {
	...
	for (int code : expectedCodes)
	    if (code == statusCode)
		return code;
	throw new HttpResponseEntityException(response, StringUtils.fastConcat("Unexpected HTTP status code: ",
		statusCode));
    }
	...
	if (!expectedContentType.getMimeType().equals(contentType.getMimeType()))
	    throw new HttpResponseEntityException(response, StringUtils.fastConcat("Wrong content type: ",
    ...
    public static HttpClient createHttpClient_AcceptsUntrustedCerts() throws KeyStoreException,
