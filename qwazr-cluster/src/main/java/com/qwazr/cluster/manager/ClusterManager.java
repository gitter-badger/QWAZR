...
public class ClusterManager {
	...
	for (String master : clusterMasterArray) {
		...
		Map<String, Set<String>> nodesMap = new ClusterSingleClient(master, 60000).getNodes();
		if (nodesMap == null)
		    continue;
    ...
    public ClusterNode upsertNode(String address, Set<String> services) throws URISyntaxException, ServerException {
     ...
     * @throws ServerException if any error occurs
     */
    public ClusterServiceStatusJson getServiceStatus(String service) throws ServerException {
	Cache cache = getNodeSetCache(service);
	if (cache == null)
	...
	if (clusterClient == null || clusterMasterArray == null || services == null || services.length == 0)
	    return;
	...
	HashMap<String, ClusterNodeSet> servicesMap = clusterNodeMap.getServicesMap();
	if (servicesMap == null)
	    return null;
